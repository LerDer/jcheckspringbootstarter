...
@Configuration
@ControllerAdvice
@EnableConfigurationProperties(ConEnvProperties.class)
public class DefaultGlobalExceptionHandler extends ResponseEntityExceptionHandler {
	...
	@Resource
	private ConEnvProperties conEnvProperties;
	...
	protected ResponseEntity handleException(HttpServletRequest request, Exception ex) {
		...
		if (ex instanceof UserFriendlyException) {
			...
			httpResult = HttpResult.failure(conEnvProperties.islocalOrDaily() ? ex.getMessage() : "参数校验错误或数据异常！");
		} else {
